...
extern void get_next_command(void *parameter);

extern void send_report(void *parameter);

extern void scan_digital_inputs(void *parameter);

extern void scan_analog_inputs(void *parameter);
...
void scan_analog_inputs(void *parameter) {
  int value;
...
void set_pin_mode() {
  ...
  switch (mode) {
    ...
    case AT_OUTPUT:
      ...
      break;
    case AT_ANALOG:
      //Serial.print("AT_ANALOG ");
      //Serial.println(pin);
      //Serial.println(can_scan);
...
void get_next_command(void *parameter) {
  ...
  byte command_r_buffer[MAX_COMMAND_LENGTH];
  ...
  while (1) {
    if (deviceConnected == true) {
      if (xQueueReceive(command_q, command_r_buffer, 10) == pdTRUE) {
        command = command_r_buffer[0];
        // get function pointer to command
        command_entry = command_table[command];
        for (int i = 0; i < 30; i++) {
          command_buffer[i] = command_r_buffer[i + 1];
        }
...
void send_report(void *parameter) {
  for (;;) {
    if (deviceConnected) {
      ...
      if (xQueueReceive(report_q, report_buffer, 10) == pdTRUE) {
        //Serial.print("send report: ");
        //for(int i = 0; i < 10; i++){
        //    Serial.print(report_buffer[i]);
        //    Serial.print(" ");
        //}
        //Serial.println();
        //Serial.println("send_report data");
        // add 1 to allow for report id
        pTxCharacteristic->setValue(report_buffer, report_buffer[0] + 1);
        pTxCharacteristic->notify();

        //Serial.println("send_report");
